...
Hoje o modal em Ativos mostra só a imagem do QR, sem identificação, e a rota `/i/:token` exige login e cai no `/login` sem explicar o contexto.

- No modal: exibir tag, nome do ativo, setor/local e a URL em texto, além de botões "Baixar PNG" e "Imprimir etiqueta".
- Na tela `/i/:token` (após scan): quando o usuário não está logado, mostrar uma tela de boas-vindas identificando o ativo escaneado ("Extintor EXT-014 — Setor Produção") antes de pedir login, em vez de redirecionar direto.
- QR inválido: mensagem indicando o que fazer (conta errada / ativo excluído).
...
Adequações que dependem de você: preciso que confirme razão social, CNPJ, endereço, e-mail do encarregado (DPO), prazo de retenção de dados após cancelamento e política de reembolso. Sem esses dados eu deixo marcadores explícitos em vez de inventar. Também recomendo, tecnicamente: exportação dos dados da empresa (portabilidade) e exclusão de conta a pedido — posso planejar depois.
...
1. Manter os preços atuais ou aplicar os propostos?
2. Confirma os dados jurídicos (razão social, CNPJ, endereço, e-mail do DPO, retenção, reembolso)?

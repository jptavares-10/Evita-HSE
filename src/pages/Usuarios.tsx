...
import { UserPlus, Copy, Clock } from "lucide-react";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { UserPermissionsDrawer } from "@/components/usuarios/UserPermissionsDrawer";
...
export default function Usuarios() {
  ...
  const handleInvite = async () => {
    ...
    setLoading(true);
    if (users.length >= company.max_users) {
      toast({ title: "Limite atingido", description: "Limite de usuários do seu plano atingido.", variant: "destructive" });
      setLoading(false);
      return;
    }
    ...
    const { data, error } = await supabase.from("invitations").insert({ company_id: profile.company_id, email: inviteEmail.trim(), invited_by: profile.id }).select().single();
    if (error || !data) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">{users.length} de {company?.max_users} usuários</p>
        </div>
        {isAdmin && (
              ...
              <TooltipTrigger asChild>
                <div>
                  <DialogTrigger asChild>
                    <Button disabled={isExpired}><UserPlus className="h-4 w-4 mr-2" />Convidar usuário</Button>
                  </DialogTrigger>
                </div>
              </TooltipTrigger>
              ...
              {inviteLink ? (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={handleCopyLink}><Copy className="h-4 w-4" /></Button>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <DialogFooter>
                    <Button onClick={handleInvite} disabled={loading || !inviteEmail.trim()}>{loading ? "Enviando..." : "Gerar convite"}</Button>
                  </DialogFooter>
                </div>
              )}
            </DialogContent>
      ...
      {/* Users table */}
      <UsersTable users={users} onClickUser={(u) => { setSelectedUser(u); setDrawerOpen(true); }} />
          ...
          <div className="space-y-2">
            {invitations.map((inv) => (
              <div key={inv.id} className="flex items-center justify-between bg-card border rounded-lg px-4 py-3">
                ...
                <Button size="sm" variant="ghost" onClick={() => handleCancelInvite(inv.id)}>Cancelar</Button>
              </div>
            ))}
          </div>
        </div>
